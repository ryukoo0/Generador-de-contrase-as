...
while True: #bucle del menu
    ...
    if opcion == '1':
        ...
        ver_contraseñas_guardadas()
        action = input ("¿Desea (c)opiar o (e)liminar alguna contraseña?")
        if action.lower() == 'c':
            idx = int(input("Ingrese el número de la contraseña a copiar: "))-1
            if 0 <= idx < len (contraseñas_guardadas):
                copiar_contraseña(contraseñas_guardadas[idx])
            else:
                print("indice no valido")
        elif action.lower() == 'e':
            eliminar_contraseña()
